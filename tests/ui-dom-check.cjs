...
test('detail uses full page, only Play Store, updated date, copy description',async()=>{await UI.openDetail(7);assert.equal(location.pathname,'/apps/7');assert.match(html(),/app-detail-panel/);assert.doesNotMatch(html(),/class="modal/);assert.doesNotMatch(html(),/Readiness/);assert.match(html(),/Updated Date/);assert.match(html(),/play\.google\.com\/store\/apps\/details/);assert.doesNotMatch(html(),/href="https:\/\/liteapks\.com/);assert.doesNotMatch(html(),/Source Page/);assert.match(html(),/data-copy-target="appDescription"/);await UI.releaseActiveDetail()});
test('per-app media refresh calls only selected app endpoint',async()=>{await UI.openDetail(7);await nodes.get('refreshAppMedia').onclick();assert.ok(calls.some(c=>c.path==='/api/apps/7/media/refresh'&&c.method==='POST'));assert.ok(!calls.some(c=>c.path==='/api/updates/start'));await UI.releaseActiveDetail()});
test('Add APK is routed and submits the supplied URL',async()=>{await UI.go('add');assert.equal(location.pathname,'/library/add');assert.match(html(),/id="sourceUrl"/);assert.doesNotMatch(html(),/class="modal/);nodes.get('sourceUrl').value='https://liteapks.com/binance.html';await nodes.get('importForm').onsubmit({preventDefault(){}});assert.ok(calls.some(c=>c.path==='/api/apps/import'&&c.body.sourcePageUrl==='https://liteapks.com/binance.html'));assert.equal(location.pathname,'/apps/7');await UI.releaseActiveDetail()});
