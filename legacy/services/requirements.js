...
function estimateProcessor(row) {
  // Architecture is already shown separately in Software Details. Processor is
  // intentionally expressed as a practical CPU minimum instead of repeating
  // x64/x86/ARM64 a second time.
  if (isHeavyProfile(row)) return '4-core CPU';
  if (isLightProfile(row)) return '2-core CPU';
  return '2-core CPU';
}

function architectureOnlyProcessor(value) {
  const raw = String(value || '').trim().toLowerCase();
  if (!raw) return true;
  return /^(?:32|64)-bit\s+(?:x86|arm)|^(?:x86|x64|arm|arm64)\b|architecture-neutral|compatible processor/.test(raw)
    || /\((?:x86|x64|arm|arm64)\).*processor/.test(raw);
}

function normalizeProcessorRequirement(value, row) {
  const raw = String(value || '').trim();
  if (!raw || architectureOnlyProcessor(raw)) return '';
  ...
  // Product pages often name Intel/AMD/Apple families instead of stating a
  // core count. The popup deliberately avoids model-name duplication and uses
  // HSWare's practical minimum in that case.
  if (/\b(intel|amd|ryzen|core\s+i[3579]|apple\s+m\d|snapdragon|xeon|athlon|celeron|pentium)\b/i.test(raw)) {
    return '';
  }
  return raw;
}
...
function withRequirementFallbacks(row) {
  ...
  const processor = normalizeProcessorRequirement(row.processor, row);
  if (!processor) estimated.processor = true;
  return {
    ...
    processor: processor || estimateProcessor(row),
