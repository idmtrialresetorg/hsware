...
const { searchWingetPackages, hasToken: hasGithubToken } = require('./github');
const { marketSeeds, intelligenceFor } = require('../catalog-intelligence');
...
function allowedForAutomaticImport(item) {
  ...
  if (isCuratedStarter(item?.packageId)) return true;
  if (looksLikeComponentOrPrerelease(item)) return false;
  // v11.4: Auto Import consumes only market-ready Catalog Intelligence
  // products. Exact/manual WinGet import remains open to the long tail.
  return intelligenceFor(item).status === 'curated';
}
...
function automaticQualification(item) {
  ...
  if (!allowedForAutomaticImport({ packageId: item?.package_id || item?.packageId, name: item?.name, publisher:item?.publisher, category: item?.category })) {
  ...
  if (score < 35) return { ok: false, score, reason: `Filtered: software quality score ${score}/100 is below the automatic-import threshold.` };
...
function normalizePackageId(value) {
  ...
  if (!packageId || packageId.length > 190 || !/^[A-Za-z0-9][A-Za-z0-9._+\-]*$/.test(packageId)) {
    throw new Error('Enter a valid WinGet Package ID.');
  }
  return packageId;
}

async function singleImportJob(id) {
  const [[row]] = await getPool().query('SELECT * FROM single_import_jobs WHERE id=? LIMIT 1', [Number(id)]);
  return row || null;
}

async function startSingleImport(packageId, requestedBy = null, source = 'manual') {
  const db = getPool();
  const cleanId = normalizePackageId(packageId);
  const cleanSource = source === 'catalog' ? 'catalog' : 'manual';
  if (cleanSource === 'catalog') {
    const [[catalogRow]] = await db.query('SELECT source_path,category,catalog_status FROM catalog_packages WHERE package_id=? LIMIT 1', [cleanId]);
    if (!catalogRow?.source_path || !catalogRow?.category) {
      throw new Error(`${cleanId} is not available in the current verified WinGet catalog. Refresh the catalog and choose a package that has a current manifest.`);
    }
  }
  const [[existing]] = await db.query('SELECT id,name,package_id FROM software WHERE package_id=? AND workspace_added=1 LIMIT 1', [cleanId]);
  if (existing) {
    return { id:null, package_id:cleanId, source:cleanSource, status:'complete', software_id:Number(existing.id), existing:true, last_error:null };
  }
  const [[active]] = await db.query(
    "SELECT * FROM single_import_jobs WHERE package_id=? AND status IN ('queued','running') ORDER BY id DESC LIMIT 1",
    [cleanId]
  );
  if (active && Number(active.requested_by || 0) === Number(requestedBy || 0)) return active;
  const [result] = await db.query(
    "INSERT INTO single_import_jobs (package_id,source,requested_by,status) VALUES (?,?,?,'queued')",
    [cleanId, cleanSource, requestedBy ? Number(requestedBy) : null]
  );
  return singleImportJob(Number(result.insertId));
}

async function stepSingleImport() {
  const db = getPool();
  const conn = await db.getConnection();
  let locked = false;
  try {
    const [[lockRow]] = await conn.query("SELECT GET_LOCK('hsware_single_import_v709',0) AS got");
    locked = Number(lockRow?.got || 0) === 1;
    if (!locked) return null;
    const [[job]] = await db.query("SELECT * FROM single_import_jobs WHERE status IN ('queued','running') AND (next_attempt_at IS NULL OR next_attempt_at<=NOW()) ORDER BY CASE WHEN status='running' THEN 0 ELSE 1 END,id LIMIT 1");
    if (!job) return null;
    const [claim] = await db.query(
      "UPDATE single_import_jobs SET status='running',started_at=COALESCE(started_at,NOW()),attempts=attempts+1,last_error=NULL WHERE id=? AND status IN ('queued','running')",
      [job.id]
    );
    if (!claim.affectedRows) return null;
    try {
      const sw = await verifiedImport(job.package_id);
      await db.query(
        "UPDATE single_import_jobs SET status='complete',software_id=?,finished_at=NOW(),next_attempt_at=NULL,last_error=NULL WHERE id=?",
        [Number(sw.id), Number(job.id)]
      );
      if (Number(job.requested_by || 0)) {
        const userId = Number(job.requested_by);
        const action = job.source === 'catalog' ? 'catalog_software_imported' : 'software_manual_imported';
        await activity.record(userId, action, { softwareId:Number(sw.id), details:{ packageId:job.package_id } });
        await notifications.notifyUser(userId, {
          actorUserId:userId, type:'success', title:'Software import completed',
          message:`${sw.name || job.package_id} was verified and added to New Software.`, softwareId:Number(sw.id),
          dedupeKey:`single-import-complete-${Number(job.id)}`
        });
        const [[actor]] = await db.query('SELECT id,name,role FROM users WHERE id=? LIMIT 1', [userId]);
        if (actor?.role === 'partner') {
          await notifications.notifyAdmins({
            actorUserId:userId, type:'info', title:'Software imported',
            message:`${actor.name || 'Partner'} imported ${sw.name || job.package_id}.`, softwareId:Number(sw.id),
            dedupeKey:`partner-single-import-${Number(job.id)}`
          }, { exceptUserId:userId });
        }
      }
    } catch (err) {
      ...
      if (attempt < 4) {
        const retryAt = new Date(Date.now() + Math.min(5 * 60_000, 15_000 * Math.pow(2, Math.max(0, attempt - 1))));
        await db.query("UPDATE single_import_jobs SET status='queued',finished_at=NULL,next_attempt_at=?,last_error=? WHERE id=?", [retryAt, message, Number(job.id)]);
      } else {
        await db.query("UPDATE single_import_jobs SET status='failed',finished_at=NOW(),next_attempt_at=NULL,last_error=? WHERE id=?", [message, Number(job.id)]);
      }
      if (Number(job.requested_by || 0)) {
        await notifications.notifyUser(Number(job.requested_by), {
          actorUserId:Number(job.requested_by), type:'warning', title:'Software import failed',
          message:`${job.package_id}: ${message.slice(0, 320)}`, dedupeKey:`single-import-failed-${Number(job.id)}`
        }).catch(() => {});
      }
    }
    return singleImportJob(Number(job.id));
  } finally {
    if (locked) { try { await conn.query("SELECT RELEASE_LOCK('hsware_single_import_v709')"); } catch {} }
    conn.release();
  }
}
  ...
  const [rows] = await db.query(
    `SELECT c.package_id,c.name,c.publisher,c.category,c.source_path,
     ...
     WHERE c.source_path IS NOT NULL AND c.source_path<>''
       AND c.category IS NOT NULL AND c.category<>'' AND c.demand_rank IS NOT NULL AND c.catalog_status='curated'
...
async function verifiedImport(packageId, options = {}) {
  ...
  try {
    const fresh = await enrichManagedSoftwareById(Number(sw.id), {
      ...
      // Initial import only needs a verified identity/version/installer plus
      // core manifest metadata. Expensive logo/site/download enrichment runs
      // later in the dedicated details worker.
      lightweight: true
    });
    ...
    if (latestHistory?.status !== 'filtered') await recordHistory(packageId, 'failed', message, options.jobId || null);
    // If the manifest vanished upstream or every stored path is stale, quarantine
    // the catalog-only row immediately. It will reappear automatically if a
    // later catalog sync sees a valid current WinGet manifest again.
    if (/Could not locate WinGet installer manifest|WinGet manifest fetch failed \(404\)/i.test(message)) {
      await db.query(
        `UPDATE catalog_packages SET source_path=NULL,metadata_status='missing',last_metadata_error=?,last_metadata_attempt_at=NOW()
         WHERE package_id=?`,
        [message.slice(0, 2000), packageId]
      ).catch(() => {});
    }
...
function startImportWorker() {
  ...
  const timer = setInterval(async () => {
    ...
    try {
      await reliability.runWorkerTick('imports', async()=>{
        const single = await stepSingleImport();
        if (!single) await stepAutoImport();
      }, {baseDelayMs:5000,maxDelayMs:5*60*1000});
    } finally { importWorkerBusy = false; }
  }, 1500);
  if (timer.unref) timer.unref();
  console.log('[HSWare] Background import worker started (single + bulk).');
}
...
async function searchCandidates(q) {
  ...
  // If the local catalog has not indexed this software yet, use a bounded,
  // authenticated GitHub code search as a fallback. This makes Manual Import
  // useful while the 50k background catalog is still being built.
  if (out.length < 12 && hasGithubToken() && needle.length >= 2) {
    const remote = await searchWingetPackages(needle, 12 - out.length);
    for (const r of remote) {
      const seedName = titleizePackageId(r.packageId);
      const seedPublisher = String(r.packageId).split('.')[0] || null;
      await db.query(
        `INSERT INTO catalog_packages (package_id,name,publisher,search_aliases,source_path,metadata_status)
         VALUES (?,?,?,?,?,'index')
         ON DUPLICATE KEY UPDATE
           name=COALESCE(NULLIF(catalog_packages.name,''),VALUES(name)),
           publisher=COALESCE(catalog_packages.publisher,VALUES(publisher)),
           search_aliases=CONCAT_WS(' ',catalog_packages.search_aliases,VALUES(search_aliases)),
           source_path=COALESCE(catalog_packages.source_path,VALUES(source_path))`,
        [r.packageId, seedName, seedPublisher, `${seedName} ${r.packageId} ${seedPublisher || ''} ${needle}`, r.sourcePath || null]
      );
      const [[cat]] = await db.query('SELECT package_id,name,publisher,category,source_path FROM catalog_packages WHERE package_id=? LIMIT 1', [r.packageId]);
      add({
        packageId:r.packageId, name:cat?.name || seedName,
        publisher:cat?.publisher || seedPublisher, category:cat?.category || null,
        sourcePath:cat?.source_path || r.sourcePath || null, managed:false, remote:true
      });
    }
  }
