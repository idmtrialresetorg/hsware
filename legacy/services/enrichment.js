...
const { enrichManagedSoftwareById, installerUrlExtension } = require('./winget');
...
async function runDetailsOnce() {
  ...
  try {
    ...
    const [[job]] = await db.query(
      `SELECT q.*,s.source_path,s.current_version,s.installer_url,s.installer_type
    ...
    try {
      ...
      const preferredInstallerFormat = String(job.installer_type || '').toLowerCase() === 'portable' ? 'portable'
        ...
        : 'all';
      await enrichManagedSoftwareById(Number(job.software_id), {
...
function startEnrichmentWorker() {
  if (started) return;
  started = true;
  const detailsTimer = setInterval(() => { reliability.runWorkerTick('enrichment',runDetailsOnce,{baseDelayMs:5000,maxDelayMs:10*60*1000}).catch(()=>{}); }, 1200);
